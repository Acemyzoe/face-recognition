...
class CNN(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = tf.keras.layers.Conv2D(
            filters=32,             # 输出空间的维数（即卷积中输出过滤器的数量）
            kernel_size=[3, 3],     # 卷积核大小
            padding='same',         # padding策略（valid 或 same）
            activation=tf.nn.relu,   # 激活函数
        )
 
        self.conv3=tf.keras.layers.Conv2D( filters=32, kernel_size=[3, 3],  activation=tf.nn.relu ) #2维卷积层，激活函数relu
        self.pool3 = tf.keras.layers.MaxPool2D(pool_size=[2, 2]) #池化层
        ...
        self.flaten1=tf.keras.layers.Flatten() #Flatten层
        self.dense3 = tf.keras.layers.Dense(units=512,activation=tf.nn.relu) #全连接层
        self.dense4 = tf.keras.layers.Dense(units=2) #全连接层（分类层）最后分类2个单位
...
if __name__ == '__main__': 
    ...
    dataset = Dataset('./face_data3/')    #数据都保存在这个文件夹下
    dataset.load()
    ...
    model.save_weights('./model/face6') #保存权重模型 命名为face1
